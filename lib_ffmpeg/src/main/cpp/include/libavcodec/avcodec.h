 ...
 * would be an invalid playState, which could put the codec user into an endless
 * loop. The API has no concept of time either: it cannot happen that trying to
 * do avcodec_send_packet() results in AVERROR(EAGAIN), but a repeated call 1 second
 * later accepts the packet (with no other receive/flush API calls involved).
 * The API is a strict playState machine, and the passage of time is not supposed
 * to influence it. Some timing-dependent behavior might still be deemed
 * acceptable in certain cases. But it must never result in both send/receive
 * returning EAGAIN at the same time at any point. It must also absolutely be
 * avoided that the current playState is "unstable" and can "flip-flop" between
 ...
 * decoder playState but are not required for output and should be dropped
 * after decoding.
 **/
 ...
 *      AVERROR(EAGAIN):   input is not accepted in the current playState - user
 ...
 *      AVERROR(EAGAIN):   output is not available in this playState - user must try
 *                         to send new input
 ...
 *              av_frame_unref(frame) before doing anything else.
 * @return 0 on success, otherwise negative ERROR code:
 *      AVERROR(EAGAIN):   output is not available in the current playState - user
...
/**
 * Reset the internal decoder playState / flush internal buffers. Should be called
...
/**
 * The bitstream filter playState.
 *
 ...
 *
 * @param opaque a pointer where libavcodec will store the iteration playState. Must
 ...
 * playState as when it was called and return a non-zero value. However,
